...
alo.svm <- function(x, y, scale = TRUE, type = NULL,
    ...
    K <- compute_svm_kernel(x, kernel_type - 1, gamma, degree, coef0, use_rfp=use_rfp)

    alpha <- rep(0, nrow(x))
    alpha[fit$index] <- fit$coefs

    alo_info <- alo_svm_kernel_rcpp(K, y, alpha, fit$rho, 1 / fit$cost, tolerance = 1e-5, use_rfp=use_rfp)
