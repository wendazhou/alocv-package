...
void svm_compute_alo_impl(blas_size n, double* K, const double* y, const double* alpha,
                          double rho, double lambda, double tol, double* alo_predicted,
                          double* alo_loss, bool use_rfp, bool use_pivoting, Config&& config) {
    ...
    config.process_response_values(y, y_pred);

    std::vector<blas_size> v_idx;
    std::vector<blas_size> s_idx;

    config.compute_active_sets(y_pred, v_idx, s_idx, tol);
    ...
    {
        const double* y_gsub = config.obtain_y(y);
        std::fill(g_sub, g_sub + n, 0.0);
        svm_compute_gsub_impl(n, nv, kv, K, n, y_gsub, s_idx, v_idx, g_sub, a_slack, format);
    }
    ...
    if(alo_loss) {
        *alo_loss = config.compute_loss(y, y_hat, a_slack);
    }
...
/**! Implementation specific bits of the SVC (classifier for support vector machines) ALO function.
 * 
 * This class implements all aspects that are specific for classifiers in computing the ALO
 * function for SVMs.
 * 
 */
class AloSvcConfig {
private:
    const blas_size n;
public:
    AloSvcConfig(blas_size n) : n(n) {}

    void process_response_values(const double* y, double* y_pred) {
        std::transform(y_pred, y_pred + n, y, y_pred, std::multiplies<double>{});
    }

    void compute_active_sets(const double* y_pred, std::vector<blas_size>& v_idx,
                             std::vector<blas_size>& s_idx, double tol) {
        for(blas_size i = 0; i < n; ++i) {
    ...
    const double* obtain_y(const double* y) {
        return y;
    }

    double compute_loss(const double* y, const double* y_hat, const double* a_slack) {
...
/** Implementation specific bits of the SVR (regression for support vector machines) ALO function.
 * 
 * This class implements all aspects that are specific for regression models in computing
 * the ALO function for SVMs.
 * 
 */
class AloSvrConfig {
private:
...
public:
    AloSvrConfig(blas_size n, double epsilon) : n(n), epsilon(epsilon) {
        y_hat_sign = static_cast<double*>(blas_malloc(16, n * sizeof(double)));
    }

    ~AloSvrConfig() {
        blas_free(y_hat_sign);
    }

    void process_response_values(const double* y, double* y_pred) {
        std::transform(y_pred, y_pred + n, y_hat_sign, [=](double x) { return std::signbit(x) ? -1 : 1; });
        std::transform(y_pred, y_pred + n, y, y_pred, [=](double x, double y) { return std::fabs(x - y); });
    }

    void compute_active_sets(const double* y_pred, std::vector<blas_size>& v_idx, std::vector<blas_size>& s_idx, double tol) {
        for(blas_size i = 0; i < n; ++i) {
            if(std::abs(y_pred[i] - epsilon) < tol) {
                v_idx.push_back(i);
            } else if (y_pred[i] >= epsilon + tol) {
                s_idx.push_back(i);
            }
        }
    }
    ...
    double compute_loss(const double* y, const double* y_hat, const double* a_slack) {
        ...
        for(blas_size i = 0; i < n; ++i) {
            accumulator += std::max(0.0, std::fabs(y[i] - (y_hat[i] + a_slack[i])) - epsilon);
        }
...
void svc_compute_alo(blas_size n, double* K, const double* y, const double* alpha,
    ...
    svm_compute_alo_impl(n, K, y, alpha, rho, lambda, tol, alo_predicted,
        alo_hinge, use_rfp, use_pivoting, AloSvcConfig{n});
}

void svr_compute_alo(blas_size n, double* K, const double* y, const double* alpha,
                     double rho, double lambda, double epsilon, double tol, double* alo_predicted, double* alo_eps,
                     bool use_rfp, bool use_pivoting) {
    svm_compute_alo_impl(n, K, y, alpha, rho, lambda, tol, alo_predicted, alo_eps, use_rfp, use_pivoting,
        AloSvrConfig{n, epsilon});
}
