...
void svr_compute_alo(blas_size n, double* K, const double* y, const double* alpha,
    ...
    double* y_pred = static_cast<double*>(blas_malloc(16, n * sizeof(double)));
    double* y_hat = static_cast<double*>(blas_malloc(16, n * sizeof(double)));
    double* y_hat_sign = static_cast<double*>(blas_malloc(16, n * sizeof(double)));
    double* a_slack = static_cast<double*>(blas_malloc(16, n * sizeof(double)));
    double* g_sub = static_cast<double*>(blas_malloc(16, n * sizeof(double)));
    ...
    // y_hat_sign now contains sign of y_hat
    std::copy(y_hat, y_hat + n, y_hat_sign);
    std::transform(y_hat_sign, y_hat_sign + n, y_hat_sign, [=](double x) { return (x > 0.0) - (x < 0.0); });
    ...
    std::fill(g_sub, g_sub + n, 0.0);
    svm_compute_gsub_impl(n, nv, kv, K, n, y_hat_sign, s_idx, v_idx, g_sub, a_slack, format);
    for(auto i : s_idx) {
        g_sub[i] = -y_hat_sign[i];
    }
